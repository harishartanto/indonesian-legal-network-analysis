...
def api_call(prompt, deployment_name, temperature, max_tokens, top_p):
    ...
    if deployment_name in ["gpt-35-turbo", "gpt-35-turbo-16k", "gpt-4"]:
        response = client.chat.completions.create(
            ...
            top_p=float(top_p),
            messages=[
                {"role": "system", "content": "You are a helpful assistant. Your task is to analyze a Indonesian legal document's set of keywords "},
...
def branch_to_str(root):
    ...
    """
    return [
        "\n".join([f"[{node.lvl}] {node.name}"] + [f"\t[{n.lvl}] {n.name}" for n in node.descendants])
        for _, _, node in RenderTree(root)
        if node.lvl == 1
    ]
...
def preprocess(text):
    '''Tokenisasi, penghapusan stopword, dan penghapusan karakter non-alphabetic'''
...
def compute_similarity(document, topics):
    '''Menghitung similaritas cosine antara dokumen dan setiap topik'''
    corpus = [document] + topics
    ...
    topics_tfidf = tfidf_matrix[1:]
    similarities = cosine_similarity(topics_tfidf, document_tfidf).flatten()
    result_dict = {topic: similarity for topic, similarity in zip(topics, similarities)}
    return result_dict